        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
            <span className="font-medium text-foreground">
              已完成 {summary.doneCount}/{summary.totalCount}
            </span>
    ...
    <div ref={rootRef} className="space-y-4">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          {viewState.viewMode === "list" && (
            <Button
              ...
              title={viewState.nestingEnabled ? "禁用父子层级" : "启用父子层级"}
            >
      ...
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && (
        <p className="text-xs text-muted-foreground">
          最多显示 {ISSUE_SEARCH_RESULT_LIMIT} 条匹配，请继续细化搜索条件。
        </p>
      )}
      {boardColumnLimitReached && (
        <p className="text-xs text-muted-foreground">
          部分看板列最多显示 {ISSUE_BOARD_COLUMN_RESULT_LIMIT} 条任务，请细化筛选或搜索查看更多结果。
        </p>
      )}
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message="没有匹配当前筛选或搜索条件的任务。"
              ...
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ...
                  : remainingIssueRowCount > 0
                    ? `已渲染 ${Math.min(renderedIssueRowLimit, filtered.length)} / ${filtered.length} 条任务`
                    : "滚动以加载更多任务"}
              </p>
            </div>
          )}
