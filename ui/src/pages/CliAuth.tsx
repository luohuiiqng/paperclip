...
export function CliAuthPage() {
  ...
  if (!challengeId || !token) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">CLI 鉴权 URL 无效。</div>;
  }

  if (sessionQuery.isLoading || challengeQuery.isLoading) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">正在加载 CLI 鉴权挑战...</div>;
  }
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">CLI 鉴权挑战不可用</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {challengeQuery.error instanceof Error ? challengeQuery.error.message : "挑战无效或已过期。"}
          </p>
        </div>
      </div>
    );
  ...
  if (!challenge) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">CLI 鉴权挑战不可用。</div>;
  }
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-xl font-semibold">
            {challenge.status === "expired" ? "CLI 鉴权挑战已过期" : "CLI 鉴权挑战已取消"}
          </h1>
          <p className="mt-2 text-sm text-muted-foreground">
            请从您的终端重新发起 CLI 鉴权流程，以生成新的审批请求。
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          本地 Paperclip CLI 进程正在请求访问此实例的董事会权限。
        </p>
        ...
        <div className="mt-5 space-y-3 text-sm">
          ...
          <div>
            <div className="text-muted-foreground">申请的访问权限</div>
            <div className="text-foreground">
              {challenge.requestedAccess === "instance_admin_required" ? "实例管理员" : "董事会"}
            </div>
          </div>
          {challenge.requestedCompanyName && (
            <div>
              <div className="text-muted-foreground">申请的公司</div>
