...
async function readLocalPackageZip(file: File): Promise<{
  ...
  if (Object.keys(archive.files).length === 0) {
    throw new Error("在所选 zip 包中未找到任何文件。");
  }
  return {
    name: file.name,
...
export function CompanyImport() {
  ...
  const localZipHelpText =
    "请上传直接由 Paperclip 导出的 .zip。使用 Finder、Explorer 或其他工具重新打包的压缩包可能无法正确导入。";
    ...
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        {sourceMode === "local" ? (
          ...
          <Field
            label="GitHub URL"
            hint="仓库树路径或指向 COMPANY.md 的 blob URL（例如 github.com/owner/repo/tree/main/company）。"
          >
            <input
          ...
          <div className="grid h-[calc(100vh-16rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
            <aside className="flex flex-col border-r border-border overflow-hidden">
              <div className="border-b border-border px-4 py-3 shrink-0">
                <h2 className="text-base font-semibold">包内文件</h2>
              </div>
