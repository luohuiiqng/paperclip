...
const concurrencyPolicyDescriptions: Record<string, string> = {
  coalesce_if_active: "当已有运行仍在进行时，仅保留一个后续运行排队。",
  always_enqueue: "每次触发都入队，即使会堆积多个运行。",
  skip_if_active: "当例行任务已在运行时，丢弃重叠触发。",
};
const catchUpPolicyDescriptions: Record<string, string> = {
  skip_missed: "忽略例行任务或调度器暂停期间错过的调度窗口。",
  enqueue_missed_with_cap: "恢复后按上限分批补齐错过的调度窗口。",
};
const signingModeDescriptions: Record<string, string> = {
  bearer: "要求在 Authorization 头中携带共享 Bearer Token。",
  hmac_sha256: "要求使用共享密钥对请求进行 HMAC SHA-256 签名。",
  github_hmac: "接受 GitHub 风格 X-Hub-Signature-256 头（对原始 body 进行 HMAC，无时间戳）。",
  none: "不做认证——Webhook URL 本身作为共享密钥。",
};
  ...
  return (
    <div className="rounded-lg border border-border p-4 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">最近：{trigger.lastResult}</span>}
...
export function RoutineDetail() {
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      if (error instanceof ApiError && error.status === 409) {
        ...
        pushToast({
          title: "例行任务已变更",
          body: "该例行任务已被其他人更新。请重新加载以查看最新版本。",
          tone: "warn",
        });
        return;
      }
      pushToast({
        title: "保存例行任务失败",
        body: error instanceof Error ? error.message : "Paperclip 无法保存该例行任务。",
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async () => {
      pushToast({ title: "例行任务运行已启动", tone: "success" });
    ...
    onError: (error) => {
      pushToast({
        title: "例行任务运行失败",
        body: error instanceof Error ? error.message : "Paperclip 无法启动该例行任务运行。",
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: "更新例行任务失败",
        body: error instanceof Error ? error.message : "Paperclip 无法更新该例行任务。",
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Repeat} message="请选择公司后查看例行任务。" />;
  }
  ...
  if (error || !routine) {
    return (
      <p className="pt-6 text-sm text-destructive">
        {error instanceof Error ? error.message : "未找到例行任务"}
      </p>
    );
  }
    ...
    <div className="max-w-2xl space-y-6">
        ...
        <div className="min-w-0 flex-1 space-y-2">
          ...
          {routine.managedByPlugin ? (
            <Badge variant="outline" className="gap-1 text-xs text-muted-foreground">
              由 {routine.managedByPlugin.pluginDisplayName} 管理
              ...
              <div key={`${entry.webhookUrl}-${index}`} className="space-y-2">
                {secretMessage.entries.length > 1 && (
                  <p className="text-xs font-medium text-muted-foreground">
                    Webhook 触发器 {index + 1} / {secretMessage.entries.length}
                  </p>
                )}
        ...
        <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              <p className="font-medium text-amber-200">已过期</p>
              <p className="text-xs text-muted-foreground">
                你编辑期间该例行任务已变更。请先重新加载并合并最新版本，再次保存。
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Button
      ...
      {!routine.assigneeAgentId ? (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-900 dark:text-amber-200">
          需要默认智能体。该例行任务可保持草稿并手动运行，但在设置默认智能体前自动化会保持暂停。
        </div>
      ) : null}
      ...
      <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
        ...
        <CollapsibleContent className="pt-3">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              <p className="text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">补齐</p>
              <Select
