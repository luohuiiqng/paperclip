...
export function InstanceSettings() {
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? `禁用 1 个定时心跳失败：${detail}`
            : `在 ${enabled.length} 个定时心跳中有 ${failures.length} 个禁用失败。首个错误：${detail}`,
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : "禁用全部心跳失败。");
    },
  });
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {grouped.length === 1 ? "家公司" : "家公司"}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              ...
              if (!window.confirm(`禁用全部 ${enabledCount} ${noun}的定时心跳？`)) {
                return;
              }
