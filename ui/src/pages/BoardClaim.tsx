...
export function BoardClaimPage() {
  ...
  if (!token || !code) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-destructive">董事会认领 URL 无效。</div>;
  }

  if (statusQuery.isLoading || sessionQuery.isLoading) {
    return <div className="mx-auto max-w-xl py-10 text-sm text-muted-foreground">正在加载认领挑战...</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          此操作会将您的用户提升为实例管理员，并将公司所有权访问从本地受信模式迁移过来。
        </p>
