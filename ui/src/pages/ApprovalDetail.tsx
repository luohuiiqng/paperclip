...
export function ApprovalDetail() {
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "驳回失败"),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "请求修改失败"),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "评论发送失败"),
  });
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">审批备注：{approval.decisionNote}</p>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              请前往 <Link to="/costs" className="underline underline-offset-2">/costs</Link> 的预算控制面板处理该预算阻断。
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm("删除这个未通过审批的智能体？此操作不可撤销。")) return;
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? "发送中…" : "发送评论"}
          </Button>
        </div>
      </div>
    </div>
